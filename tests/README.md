...
The fixtures cover document formats 0–6 and use synthetic catalog data only. They are public compatibility contracts and must remain importable when the schema changes. The format-5 fixture includes a synthetic change-history entry, while format 6 adds stable built-in-scenario linkage and its catalog version so persistence, validation, import, and export remain covered.
