...
Die 42 Gegnerprofile der 21 eingebauten Szenarien sind ohne Import verfügbar und erscheinen automatisch nach der Szenarioauswahl. Der optionale Gegnerkatalog unter **Load Enemy Catalog** bleibt davon getrennt: Er dient dazu, Gegner samt vorgeschlagenem XP-Wert bequem als Kill-Zeile zur aktiven Mission hinzuzufügen. **Add blank enemy** bleibt für eigene oder szenariospezifische Gegner verfügbar. Wo ein Szenario keinen allgemeinen Gegner-XP-Wert vorgibt, zeigt die Referenz **Scenario XP**. Die **Outcome and Experience**-Regeln des gespielten Szenarios entscheiden weiterhin, was tatsächlich zählt; Name und Wert bleiben deshalb in jedem Missionsbericht manuell korrigierbar.
...
Der **Turn Tracker** erscheint bei jedem eingebauten Szenario in Edit und Play. Große `−/+`-Tasten ändern den aktuellen Zug; der Hinweis daneben berücksichtigt Ereignisse in jeder Runde, nur in ungeraden Runden, an festen Zügen, beim Öffnen neuer Räume, ab oder bis zu einem bestimmten Zug, bei Bedingungen und gedruckte Ausnahmen. Das Erreichen oder Überschreiten eines Zuglimits wird deutlich markiert, aber nicht blockiert. Der Zugstand wird mit dem Missionsbericht gespeichert und exportiert; der Tracker erinnert nur und führt keine Ereignisse selbst aus.
